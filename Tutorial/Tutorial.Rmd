...
Now that we know what the API responds when asking for indicators metada, we are going to select what indicator we are going to ask the API about. The next steps in chapter 4.1.5 will guide you through how to analyse and search this indicators with code, but there is an alternative here. In the next **code chunk** we are storing the **meta_ind_gender** DataFrame into a CVS file (command separated values) that you can open in Excell and examine all the gender indicators to decide which ones you are interested at.
```{r}
# Write CSV in R
...
--> explanations
```{r}
female_prim <- subset(work_data, IndicatorCode == "SOC_1328")
#female_prim <- subset(work_data, IndicatorCode == "SOC_1328" & (CountryCode == 'ARG' | CountryCode == 'BOL' | CountryCode == 'BRA' | CountryCode == 'CHL')) 
head(female_prim)
...
#####4.2.3. Plotting - comparing indicators by country
...
head(work_data_2016)
unique(work_data_2016$CountryCode)
unique(work_data_2016$IndicatorCode)
...
head(work_data)
max(work_data$Year)
edu_data_2016 <- work_data[work_data$Year == "2016" & (work_data$IndicatorCode == "SOC_1328" | work_data$IndicatorCode =="SOC_686"), ]
head(edu_data_2016)
unique(edu_data_2016$CountryCode)
unique(edu_data_2016$IndicatorCode)
unique(edu_data_2016$Year)
edu_data_2016$Gender <- ifelse(edu_data_2016$IndicatorCode == "SOC_1328", "Female", "Male")
head(work_data_2016)
```

```{r}
#write.csv(edu_data_2016, file="edu_data_2016.csv")
```




#####4.2.4. Plotting - multiple plots
